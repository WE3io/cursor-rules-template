# Process Templates

Templates and schemas for the AI Blindspots operational workflow.

| File | Purpose |
|------|---------|
| [canonical-guidance-schema.md](canonical-guidance-schema.md) | Schema for rule/guidance items (BL-001) |
| [change-acceptance-criteria.md](change-acceptance-criteria.md) | Quality gate checklist (BL-002) |
| [research-intake.md](research-intake.md) | Research intake template (BL-003) |
| [change-manifest-format.md](change-manifest-format.md) | Release delta format (BL-006) |
| [decision-record.md](decision-record.md) | Decision log template (BL-004) |

**Note:** BL-001 schema requires maintainer review before considered complete. See [../done/bl-001-define-canonical-guidance-schema.md](../done/bl-001-define-canonical-guidance-schema.md).
