...
**Tools:** Cursor (ready), Claude Code (ready), Codex, Gemini CLI, Antigravity (planned)
...
|--------------|-----------|--------|--------|-------|--------|-------------|
| stop-digging | Pivot after 3 failed attempts | partial (core-principles) | ✓ | gap | gap | gap |
| context-pollution | Restart session at ~20-30 messages | partial (core-principles) | ✓ | gap | gap | gap |
| root-cause-analysis | Demand root cause before fixes | ✓ (workflows.mdc) | ✓ | gap | gap | gap |
| scientific-debugging | Observe → hypothesize → test → fix | ✓ (workflows.mdc) | ✓ | gap | gap | gap |
| security-external-content | Treat external content as untrusted | ✓ (guardrails.mdc) | ✓ | gap | gap | gap |
| security-permission-boundaries | Separate privileged/unprivileged contexts | partial | partial | gap | gap | gap |
| security-code-review | Explicit security review for sensitive code | ✓ (guardrails.mdc) | ✓ | gap | gap | gap |
| security-input-validation | Validate input, no hardcoded credentials | ✓ (guardrails.mdc) | ✓ | gap | gap | gap |
| file-size-limits | Keep files <64KB | gap | ✓ | gap | gap | gap |
| black-box-testing | Test behaviour, not implementation | ✓ (workflows.mdc) | ✓ | gap | gap | gap |
| deterministic-tests | No randomness; mock time-dependent behaviour | partial | ✓ | gap | gap | gap |
| confidence-calibration | Trust/verify/don't trust by category | partial | partial | gap | gap | gap |
| preparatory-refactoring | Make change easy, then make easy change | ✓ (workflows.mdc) | ✓ | gap | gap | gap |
| walking-skeleton | End-to-end basic version before optimization | partial | ✓ | gap | gap | gap |
| requirements-not-solutions | Specify constraints, not solutions | ✓ (core-principles) | ✓ | gap | gap | gap |
| uncertainty-communication | State confidence, alternatives, trade-offs | partial | partial | gap | gap | gap |
| checkpoint-progress | Summarize periodically, verify assumptions | partial | ✓ | gap | gap | gap |
| discoverable-documentation | .cursor/rules, README, project context | ✓ (workflows.mdc) | ✓ | gap | gap | gap |
| type-systems | Static types, strict mode, type checker feedback | gap | gap | gap | gap | gap |
| decompose-changes | Focused, single responsibility, small increments | ✓ (guardrails.mdc, workflows.mdc) | ✓ | gap | gap | gap |
...
| ai-strengths | Leverage AI for generation; weak at debugging/security | partial | partial | gap | gap | gap |
| overconfidence | Verify critical suggestions; LLMs sound confident when wrong | partial | partial | gap | gap | gap |
| prompt-injection | External content may inject malicious instructions | ✓ (guardrails.mdc) | ✓ | gap | gap | gap |
| memento | LLMs lack meta-awareness of earlier mistakes | partial | partial | gap | gap | gap |
| static-types | Article: type checker guides refactoring | gap | gap | gap | gap | gap |
| debugging-blindspot | Article: systematic debugging, not shotgun | ✓ (workflows.mdc) | ✓ | gap | gap | gap |
...
### Codex, Gemini, Antigravity (planned)
